...
async function main() {
  ...
  const res = await axios.get('https://api.sorcerytcg.com/api/cards', { timeout: 60000 });
  ...
  try {
    ...
    fs.writeFileSync(path.join(dataDir, 'cards_raw.json'), JSON.stringify(cards, null, 2));
    console.log('Saved raw snapshot to data/cards_raw.json');
  } catch (e) {
    console.warn('Warning: could not write data/cards_raw.json:', e?.message || e);
  }

  let createdCards = 0, updatedCards = 0, createdSets = 0, createdVariants = 0;
  ...
  for (const card of cards) {
    ...
    if (!dbCard) {
      dbCard = await prisma.card.create({ data: { name, elements: elements || null, subTypes: subTypes || null } });
      createdCards++;
    } else {
      // update basic fields if changed
      if (dbCard.elements !== elements || dbCard.subTypes !== subTypes) {
        dbCard = await prisma.card.update({ where: { id: dbCard.id }, data: { elements: elements || null, subTypes: subTypes || null } });
        updatedCards++;
      }
    }
    ...
    if (Array.isArray(sets)) {
      for (const s of sets) {
        ...
        if (!dbSet) {
          dbSet = await prisma.set.create({ data: { name: setName, releasedAt } });
          createdSets++;
        }
  ...
  console.log(`Done. Cards created: ${createdCards}, updated: ${updatedCards}; Sets created: ${createdSets}; Variants created: ${createdVariants}`);
}
