...
describe('Mana Providers', () => {
  describe('MANA_PROVIDER_BY_NAME', () => {
    ...
    it('should contain known mana providers', () => {
      ...
      expect(MANA_PROVIDER_BY_NAME.has('tintagel')).toBe(true);
    });
    ...
    it('should contain Site cards as mana providers', () => {
      expect(MANA_PROVIDER_BY_NAME.has('glastonbury tor')).toBe(true);
      expect(MANA_PROVIDER_BY_NAME.has('caerleon-upon-usk')).toBe(true);
      expect(MANA_PROVIDER_BY_NAME.has('joyous garde')).toBe(true);
      expect(MANA_PROVIDER_BY_NAME.has('pristine paradise')).toBe(true);
    });

    it('should contain Permanent cards as mana providers', () => {
      expect(MANA_PROVIDER_BY_NAME.has('common cottagers')).toBe(true);
      expect(MANA_PROVIDER_BY_NAME.has('castle servants')).toBe(true);
      expect(MANA_PROVIDER_BY_NAME.has('blacksmith family')).toBe(true);
    });
    ...
    it('should have at least 20 entries', () => {
      // There are currently 27 known mana providers
      expect(MANA_PROVIDER_BY_NAME.size).toBeGreaterThanOrEqual(20);
    });
  ...
  describe('THRESHOLD_GRANT_BY_NAME', () => {
    ...
    it('should only contain mana providers that are also in MANA_PROVIDER_BY_NAME', () => {
      const thresholdCardNames = Object.keys(THRESHOLD_GRANT_BY_NAME);

      for (const cardName of thresholdCardNames) {
        expect(MANA_PROVIDER_BY_NAME.has(cardName)).toBe(true);
      }
    });
    ...
    it('should have exactly 4 Core cards', () => {
      expect(Object.keys(THRESHOLD_GRANT_BY_NAME).length).toBe(4);
    });
  ...
  describe('Integration Tests', () => {
    it('should correctly identify all Cores as both mana and threshold providers', () => {
      const cores = ['amethyst core', 'aquamarine core', 'onyx core', 'ruby core'];
    ...
    it('should have mana providers without threshold grants', () => {
      ...
      const manaProvidersWithoutThresholds = Array.from(MANA_PROVIDER_BY_NAME).filter(
        name => !THRESHOLD_GRANT_BY_NAME[name]
      );
    ...
    it('should not have duplicate entries across data structures', () => {
      // All keys in THRESHOLD_GRANT_BY_NAME should be in MANA_PROVIDER_BY_NAME
      const thresholdKeys = Object.keys(THRESHOLD_GRANT_BY_NAME);
      const manaProviders = Array.from(MANA_PROVIDER_BY_NAME);

      for (const key of thresholdKeys) {
        expect(manaProviders).toContain(key);
      }
    });
  ...
  describe('Data Consistency', () => {
    ...
    it('should not have null or undefined in mana providers', () => {
      ...
      expect(MANA_PROVIDER_BY_NAME.has(undefined as unknown as string)).toBe(false);
    });
    ...
    it('should have consistent naming format', () => {
      ...
      const allConsistent = Array.from(MANA_PROVIDER_BY_NAME).every(
        name => {
