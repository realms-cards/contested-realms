...
describe("Draft Event Compatibility", () => {
  ...
  describe("Existing draft functionality preservation", () => {
    it("should maintain makeDraftPick functionality with enhanced UI", async () => {
      ...
      expect(transport.mockSocket.emit).toHaveBeenCalledWith(
        "makeDraftPick",
        pickConfig
      );
    });
    ...
    it("should maintain submitDeck functionality for deck building phase", async () => {
      ...
      const testDeck = {
        ...
        sideboard: [{ cardId: "card-004", quantity: 1 }],
      };
      ...
      // Assert
      expect(transport.mockSocket.emit).toHaveBeenCalledWith("submitDeck", {
        deckData: testDeck,
        playerId: "player-1",
      });
    });
  ...
  describe("Online draft events integration", () => {
  ...
  describe("Backward compatibility validation", () => {
    it("should work with clients that do not support enhanced events", async () => {
      ...
      expect(legacyTransport.mockSocket.emit).toHaveBeenCalledWith(
        "makeDraftPick",
        pickConfig
      );
    });
    ...
    it("should gracefully handle mixed client versions in same session", async () => {
      ...
      expect(
        enhancedUpdates.find((u) => u.type === "draft")?.data.picks[0]
      ).toContain("mixed-session-card-001");
      expect(
        enhancedUpdates.find((u) => u.type === "preview")?.data.cardId
      ).toBe("mixed-session-card-001");
    });
  });
