...
describe("precon Genesis", () => {
  ...
  it("skips a queued draw Genesis when its minion leaves before resolution", async () => {
    ...
    const choices = getSpellChoices(store.getState(),"p1","Apprentice Wizard");
    expect(choices.map(choice => choice.key)).toEqual(["genesis/gone"]);
  ...
  it("lets the human order simultaneous Genesis triggers, after the non-active CPU's triggers", async () => {
    const store = setup();
    store.setState({matchId:"genesis-order",transport:new LocalTransport(),zones:{...store.getState().zones,
    ...
    const selected = store.getState().cpuTriggerOptions?.[1];
    if (!selected) throw new Error("Missing simultaneous trigger choice");
    store.getState().chooseCpuTrigger(selected.id);
    await Promise.resolve();
    const firstEvent = store.getState().pendingMagic?.cpuEvent;
    expect(firstEvent?.kind === "genesis" && firstEvent.source).toEqual(expect.objectContaining({instanceId:"second-wizard"}));
    store.getState().setCpuMagicChoice("genesis/draw");
    store.getState().resolveMagic();
    await Promise.resolve();
    const secondEvent = store.getState().pendingMagic?.cpuEvent;
    expect(secondEvent?.kind === "genesis" && secondEvent.source).toEqual(expect.objectContaining({instanceId:"Apprentice Wizard"}));
  });
  it("interrupts a multi-step effect for Genesis before drawing and completing the original spell", async () => {
    ...
    expect(store.getState().cpuEffectContinuations).toHaveLength(0);
    expect(transport.sendMessage).toHaveBeenCalledWith(expect.objectContaining({type:"magicResolve",id:"outer-spell"}));
  });
  ...
  it("queues entry once, resolves the human choice, and ignores a state echo", async () => {
    const store = setup();
