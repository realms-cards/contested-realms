...
export class TournamentStatisticsService {
   ...
   */
  async getTournamentStatistics(tournamentId: string): Promise<TournamentStatisticsResponse> {
   ...
   */
  async calculateStandings(tournamentId: string): Promise<TournamentStanding[]> {
    ...
    const standingsWithTiebreakers = await Promise.all(
      playerStandings.map(async (standing) => {
        const tiebreakers = await this.calculateTiebreakers(tournamentId, standing.playerId);
    ...
    for (let i = 0; i < sortedStandings.length; i++) {
      if (i > 0 && !this.areStandingsEqual(sortedStandings[i-1], sortedStandings[i])) {
        currentRank = i + 1;
      }
   ...
   */
  async calculateTiebreakers(tournamentId: string, playerId: string): Promise<TiebreakersCalculation> {
    ...
    // Calculate player's game win percentage
    let totalGames = 0;
    let gamesWon = 0;
    ...
    for (const match of matches) {
      ...
      if (results && 
        ...
        const playerWins = isPlayer1 ? results.player1Wins : results.player2Wins;
        const playerLosses = isPlayer1 ? results.player2Wins : results.player1Wins;
    ...
    for (const opponentId of opponentIds) {
      const opponentStanding = await prisma.playerStanding.findUnique({
        where: {
      ...
      if (opponentStanding) {
        ...
        const opponentMatchWinPercentage = opponentMatches > 0 ? 
          Math.max(0.33, opponentStanding.wins / opponentMatches) : 0.33; // Minimum 33% per MTG rules

        totalOpponentMatchWinPercentage += opponentMatchWinPercentage;

        // Calculate opponent's game win percentage
        const opponentTiebreakers = await this.calculateOpponentGameWinPercentage(tournamentId, opponentId);
        totalOpponentGameWinPercentage += Math.max(0.33, opponentTiebreakers);

        opponentCount++;
      }
    }

    const opponentMatchWinPercentage = opponentCount > 0 ? 
      totalOpponentMatchWinPercentage / opponentCount : 0;
    
    const opponentGameWinPercentage = opponentCount > 0 ? 
      totalOpponentGameWinPercentage / opponentCount : 0;

    return {
      opponentMatchWinPercentage,
      gameWinPercentage: Math.max(0.33, gameWinPercentage), // Minimum 33%
   ...
   */
  private async calculateOpponentGameWinPercentage(tournamentId: string, playerId: string): Promise<number> {
    const matches = await prisma.match.findMany({
      where: {
        tournamentId,
        players: {
   ...
   */
  async getTournamentRounds(tournamentId: string): Promise<TournamentRoundResponse[]> {
          ...
          round.matches.map(async (match) => {
            ...
            if (matchPlayers.length > 0) {
              const player1 = await prisma.user.findUnique({
                where: { id: String(matchPlayers[0]) },
                select: { name: true }
              });
              ...
              if (matchPlayers.length > 1) {
                const player2 = await prisma.user.findUnique({
                  where: { id: String(matchPlayers[1]) },
                  select: { name: true }
                });
            ...
            return {
              id: match.id,
              player1Id: matchPlayers[0] ? String(matchPlayers[0]) : '',
              player1Name,
              player2Id: matchPlayers[1] ? String(matchPlayers[1]) : '',
              player2Name,
              status: match.status as 'pending' | 'active' | 'completed' | 'cancelled',
              result: results ? {
                winnerId: (typeof results.winnerId === 'string' ? results.winnerId : null),
                player1Wins: (typeof results.player1Wins === 'number' ? results.player1Wins : 0),
                player2Wins: (typeof results.player2Wins === 'number' ? results.player2Wins : 0),
  ...
  async calculateOverallStats(tournamentId: string) {
    ...
    const completedMatchesWithTimes = matches.filter(m => 
    ...
    if (completedMatchesWithTimes.length > 0) {
      const totalDuration = completedMatchesWithTimes.reduce((sum, match) => {
        if (match.startedAt && match.completedAt) {
          return sum + (match.completedAt.getTime() - match.startedAt.getTime());
        }
        return sum;
      }, 0);
      averageMatchDuration = Math.round(totalDuration / completedMatchesWithTimes.length / 1000); // seconds
    }
    ...
    const roundsCompleted = tournament.rounds.filter(r => r.status === 'completed').length;
  ...
  async updatePlayerStanding(
    ...
    const matchPoints = matchResult.won ? SWISS_PAIRING.MATCH_WIN_POINTS : 
                      matchResult.draw ? SWISS_PAIRING.MATCH_DRAW_POINTS : 
   ...
   */
  private async updatePlayerTiebreakers(tournamentId: string, playerId: string): Promise<void> {
   ...
   */
  private areStandingsEqual(a: TournamentStanding, b: TournamentStanding): boolean {
    return a.matchPoints === b.matchPoints &&
           Math.abs(a.tiebreakers.opponentMatchWinPercentage - b.tiebreakers.opponentMatchWinPercentage) < 0.001 &&
           Math.abs(a.tiebreakers.gameWinPercentage - b.tiebreakers.gameWinPercentage) < 0.001 &&
           Math.abs(a.tiebreakers.opponentGameWinPercentage - b.tiebreakers.opponentGameWinPercentage) < 0.001;
  }
