...
export function useRealtimeTournamentPreparation(
  tournamentId: string,
  opts?: { isConnected?: boolean; pollIntervalMs?: number }
) {
  ...
  useEffect(() => {
    refreshStatus();
    const pollMs = opts?.pollIntervalMs ?? 45000; // Increased for cost savings
