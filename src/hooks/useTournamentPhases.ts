...
const PHASE_DEFINITIONS: Record<TournamentStatus, TournamentPhase["phase"]> = {
  registering: {
    ...
    description:
      "Players are joining the tournament and setting their ready status",
    autoTransition: false,
  },
  preparing: {
    name: "Preparation Phase",
    description:
      "Players are preparing for the tournament (opening packs, drafting, selecting decks)",
...
export function useTournamentPhases(
  ...
  const checkConditions = useCallback(async () => {
    ...
    try {
      ...
      const [tournamentResponse, standingsResponse, preparationResponse] =
      ...
      if (
        tournamentResponse.status === "fulfilled" &&
        tournamentResponse.value.ok
      ) {
      ...
      if (
        ...
        standingsResponse.value.ok
      ) {
        const standings = await standingsResponse.value.json();
        conditions.all_players_ready = standings.standings.every(
          (s: { isReady?: boolean }) => s.isReady
        );
      }

      // Check preparation conditions
      if (
        preparationResponse.status === "fulfilled" &&
        preparationResponse.value.ok
      ) {
      ...
      setState((prev) => ({
        ...prev,
        error:
          err instanceof Error ? err.message : "Failed to check conditions",
        loading: false,
      }));
    }
  ...
  const performAction = useCallback(
    async (action: string): Promise<Response | undefined> => {
      // Skip API calls if no tournament ID
      if (!tournamentId) {
        return undefined;
      }
      ...
      const transition = PHASE_TRANSITIONS.find(
        (t) => t.from === state.currentPhase.status && t.action === action
      );
    ...
    [
      tournamentId,
      state.currentPhase.status,
      state.canPerformActions,
      updatePhase,
    ]
  );
  ...
  useEffect(() => {
    ...
    checkConditions();
    const pollMs = options?.pollIntervalMs ?? 45000; // Increased for cost savings
