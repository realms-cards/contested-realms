...
export async function POST(
  ...
  function replaceIdDeep(
    ...
    if (Array.isArray(value))
      return value.map((item) => replaceIdDeep(item, fromId, toId));
  ...
  function containsIdDeep(value: unknown, id: string): boolean {
    ...
    if (Array.isArray(value))
      return value.some((item) => containsIdDeep(item, id));
    if (value && typeof value === "object") {
  ...
  function replacePlayerDecks(
    ...
    if (
      !playerDecks ||
      typeof playerDecks !== "object" ||
      Array.isArray(playerDecks)
    )
      return playerDecks;
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({ error: "Tournament already finished" }),
        { status: 400 },
      );
    }
      ...
      if (existingRegistration.seatStatus === "vacant") {
        await prisma.tournamentRegistration.update({
          ...
          data: {
            seatStatus: "active",
            seatMeta: {
              ...((existingRegistration.seatMeta as Record<
                string,
                unknown
              > | null) ?? {}),
    ...
    if (
      isOpenSeat &&
      tournament.registrations.length >= TOURNAMENT_PLAYER_LIMITS.MAX_PLAYERS
    ) {
      return new Response(
        JSON.stringify({
    ...
    if (existingTournamentRegistrations.length > 0) {
      const tournamentName =
        existingTournamentRegistrations[0]?.tournament?.name;
      return new Response(
        JSON.stringify({
    ...
    if (isOpenSeat && !canOpenSeatJoinNewSeat) {
      const vacantSeat = tournament.registrations.find(
        (reg) => reg.seatStatus === "vacant",
      );
      if (!vacantSeat) {
        return new Response(
          JSON.stringify({
      ...
      if (!vacantSeat.deckSubmitted) {
        return new Response(
          JSON.stringify({ error: "Vacant seat has no deck to inherit yet" }),
          { status: 400 },
        );
      }
      ...
      // Allow replacement even if the vacant seat has an active match
      // The new player will take over the match in progress
      // This is especially important for sealed/draft where they inherit the deck
      const activeMatchId = vacantStanding?.currentMatchId || null;

      const seatMeta =
        (vacantSeat.seatMeta as Record<string, unknown> | null) ?? {};
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (existingStanding) {
          await tx.playerStanding.update({
            ...
            data: {
              ...
              displayName,
              isEliminated: false,
              // Keep the active match ID so the new player takes over the match in progress
        ...
        for (const match of matches) {
          ...
          const resultsHasId = containsIdDeep(
            match.results,
            vacantSeat.playerId,
          );
          const decksHasId =
      ...
      // Create registration
      console.log("Creating tournament registration:", {
        tournamentId: id,
        playerId: userId,
        displayName,
      });
      ...
      console.log("Upserting player standing:", {
        tournamentId: id,
        playerId: userId,
        displayName,
      });
