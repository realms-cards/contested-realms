  ...
  preparationData: z.object({
    sealed: z
      .object({
        ...
        // Sideboard cards become collection in limited formats
        sideboardList: z
          .array(
            z.object({
              cardId: z.string(),
...
export async function POST(
  ...
  try {
    ...
    if (registration.preparationStatus !== "inProgress") {
      return new Response(
        JSON.stringify({
          error: "Preparation not started or already completed",
        }),
        { status: 400 },
      );
    }
    ...
    switch (format) {
      case "sealed":
        if (!preparationData.sealed) {
          return new Response(
            JSON.stringify({ error: "Sealed preparation data required" }),
            { status: 400 },
          );
        }
        ...
        const sealedData = preparationData.sealed;
        {
          ...
          isComplete =
            sealedData.packsOpened &&
            sealedData.deckBuilt &&
            total >= MIN_DECK_CARDS;
        }
        deckSubmitted = isComplete;

        if (isComplete && !validateDeckList(sealedData.deckList)) {
          return new Response(JSON.stringify({ error: "Invalid deck list" }), {
            status: 400,
          });
        }
        break;
      ...
      case "draft":
        if (!preparationData.draft) {
          return new Response(
            JSON.stringify({ error: "Draft preparation data required" }),
            { status: 400 },
          );
        }
    ...
    const updatedPrepData = {
      ...
      ready: isComplete
        ? true
        : ((currentPrepData as { ready?: boolean })?.ready ?? false),
