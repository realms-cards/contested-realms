...
export async function GET(
  ...
  try {
    ...
    const [tournament, standings, statistics, rounds] = await Promise.all([
      ...
      prisma.playerStanding.findMany({
      ...
      prisma.tournamentStatistics.findMany({
        where: { tournamentId: id },
        include: {
          player: {
            select: { name: true, image: true },
    ...
    const formatStats = {
      ...
      dropoutRate:
        standings.filter((s) => s.isEliminated).length / totalPlayers,
    };
      ...
      JSON.stringify({
        ...
        overview: {
          ...
          completedMatches: rounds.reduce(
            (sum, round) =>
              sum + round.matches.filter((m) => m.results !== null).length,
            0
          ),
        ...
        rounds: rounds.map((round) => ({
          ...
          completedMatches: round.matches.filter((m) => m.results !== null)
            .length,
