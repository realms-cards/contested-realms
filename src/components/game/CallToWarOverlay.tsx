...
export default function CallToWarOverlay() {
  ...
  const selectCallToWarCard = useGameStore((s) => s.selectCallToWarCard);
  const resolveCallToWar = useGameStore((s) => s.resolveCallToWar);
  const cancelCallToWar = useGameStore((s) => s.cancelCallToWar);
  ...
  const handleSelectCard = useCallback(
    (index: number) => {
      ...
      selectCallToWarCard(index);
    },
    [isCaster, pending?.phase, selectCallToWarCard]
  );
  ...
  }, [resolveCallToWar, selectedIndex]);
          ...
          <span className="opacity-80">
            {phase === "selecting" &&
              (isCaster
                ...
                : `${pending.casterSeat.toUpperCase()} is searching for warriors...`)}
